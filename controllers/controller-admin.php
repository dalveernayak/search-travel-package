...
class Tg_Searchboxes_Controller_Admin extends Tg_Searchboxes_Controller_Base {
	/**
		...
		* @var string
	*/
	private $page_slug	= 'nayak_searchboxes';
	/**
...
//		add_action('admin_init', array($this, 'jquery_ui'), 0);
		...
		if($doing_ajax && $valid_ajax_call && isset($_GET['action']) && 'nayak_searchboxes' == $_GET['action'] ) {
	...
	function tg_searchboxes_create_menu() {
		...
		$searchboxesSettingsPage	= add_menu_page(
			// page title
			'search travel package Search &rsaquo; Default Settings',
			// menu title
...
//			array($this, 'tg_searchboxes_settings'),
	...
	function tg_searchboxes_load_view($page_type) {
		...
		$views	= array('settings', 'nayak_searchboxes', 'demo');
		...
		$headlines	= array(
			/*	settings page	*/
			'settings'	=> 'Nayak Search &rsaquo; Default Settings',
			/*	box loaded via AJAX into thickbox	*/
			'nayak_searchboxes'=> 'Nayak Searchboxes',
			/*	demo page	*/
			'demo'		=> 'Nayak Searchboxes &rsaquo; shortcode Page');
...
//        $suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '.dev' : '';
	...
	function do_action_tg_searchboxes() {
		//testing if the current request was referred from an admin page
     		check_admin_referer( $this->get_nonce( 'nayak_searchboxes' ) );
     		// loading the corespondent view
		$this->tg_searchboxes_load_view( 'nayak_searchboxes' );
